...
public class SingleMapsActivity extends FragmentActivity implements OnMapReadyCallback {
    ...
    private void clearTotals(){
        total_green_pin_info.setText(getString(R.string.total_green_pins, 0));
        total_yellow_pin_info.setText(getString(R.string.total_yellow_pins, 0));
        total_red_pin_info.setText(getString(R.string.total_red_pins, 0));
    }
        ...
        if (report != null){
                switch (Objects.requireNonNull(report.getLevel())){
                    case "🟢 ต่ำ - ไม่เร่งด่วน":
                        total_green_pin_info.setText(getString(R.string.total_green_pins, 1));
                        break;
                    case "🟡 ปานกลาง - ต้องการความช่วยเหลือเร่งด่วน":
                        total_yellow_pin_info.setText(getString(R.string.total_yellow_pins, 1));
                        break;
                    case "🔴 วิกฤติ - อันตรายถึงชีวิต":
                        total_red_pin_info.setText(getString(R.string.total_red_pins, 1));
                        break;
                }
        }
    }
    ...
    private void setup(){
        total_pins_info.setVisibility(View.GONE);
    }
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        if (report != null) {
            Report reportLocation = report;
            LatLng reportPos = new LatLng(reportLocation.getLocation().lat, reportLocation.getLocation().lng);
            mMap.moveCamera(CameraUpdateFactory.newLatLngZoom(reportPos, 12));
        }
    }

    private void addMarkersToMap() {
        if (!isMapReady || mMap == null || report == null) {
            return;
        }
        mMap.clear();
        markerMap.clear();
